    ...
    if item.components.edible ~= nil then
        ...
        if item.components.edible.foodtype == FOODTYPE.WOOD then
            ...
            elseif giver.components.leader ~= nil then
                ...
                inst.components.follower:AddLoyaltyTime((item.components.edible.foodtype == FOODTYPE.WOOD and 15*TUNING.PIG_LOYALTY_PER_HUNGER or 0))
...
local function OnEat(inst, food)
    if food.components.edible ~= nil then
        if food.components.edible.foodtype == FOODTYPE.WOOD then
            --inst.treesdue = inst.treesdue + 1
            inst.woodmeter = inst.woodmeter + 15 --food.components.edible.woodiness
            --CraftWall(inst)
...
local function MakeWoodEater(eater)
	local old_PrefersToEat = eater.PrefersToEat
	local new_PrefersToEat = function(self,food)
		if food ~= nil and food.components.edible ~= nil and food.components.edible.foodtype == FOODTYPE.WOOD then
			return true
		end
		return old_PrefersToEat(self,food)
	end
	eater.PrefersToEat = new_PrefersToEat
end
...
local function fn()
	...
	MakeWoodEater(inst.components.eater)
